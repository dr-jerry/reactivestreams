  ...
  def receiveN(requester: TestProbe, ops: Seq[Operation], expectedReplies: Seq[OperationReply]): Unit =
    requester.within(5.seconds) {
      val repliesUnsorted = for (i <- 1 to ops.size) yield try {
        ...
        //case ex: Throwable if ops.size > 10 => succeed(s"failure to receive confirmation $i/${ops.size}", ex)
        case ex: Throwable                  => OperationMissed(1000 + i,ops.size, i) //(s"failure to receive confirmation $i/${ops.size}\nRequests:" + ops.mkString("\n    ", "\n     ", ""), ex)
      }
